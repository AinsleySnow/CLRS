using System;

class Program
{
    static int Main()
    {
        int[] array = {5,4,3,2,1};
        int b = FindInversions(ref array,0,4);
        Console.WriteLine(b);
        return 0;
    }

    static int Merge(ref int[] array, int left, int middle, int right)
    {
        int[] leftNums = new int[middle - left + 2];
        int[] rightNums = new int[right - middle + 1];
        
        int ans = 0;
        
        for(int i = 0; i < leftNums.Length - 1; ++i)
            leftNums[i] = array[left + i];
        for(int i = 0; i < rightNums.Length - 1;++i)
            rightNums[i] = array[middle + i + 1];
        
        leftNums[leftNums.Length - 1] = int.MaxValue;
        rightNums[rightNums.Length - 1] = int.MaxValue;

        int _1 = 0, _2 = 0;
        for(int i = left; i <= right; ++i)
        {
            if(leftNums[_1] <= rightNums[_2])
                array[i] = leftNums[_1++];
            else
            {
                ans += leftNums.Length - _1 - 1;
                array[i] = rightNums[_2++];
            }    
        }
        return ans;
    }

    static int FindInversions(ref int[] array,int left,int right)
    {
        if(left < right)
        {
            int middle = (left + right) / 2, ans = 0;
            ans += FindInversions(ref array, left, middle);
            ans += FindInversions(ref array, middle + 1, right);
            ans += Merge(ref array, left, middle, right);
            return ans;
        }
        else
            return 0;
    }
}