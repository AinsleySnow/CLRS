...
vertex_list find_g_square_list(vertex_list g)
{
    int n = sizeof(g) / sizeof(graph_node);
    ...
    for (int i = 0; i < n; ++i)
    {
        if (g[i].c == white)
        {
            ...
            while (current)
            {
                ...
                list_append(g_square[i].adj, current);
    ...
    for (int i = 0; i < n; ++i)
    {
        if (g[i].c == gray)
        {
            ...
            {
                list_append(g_square[i].adj, current);
                list_append((g_square + (g[i].pi - g))->adj, current);
            }
        }
    }
...
int** find_g_square_matrix(int** g)
{
...
int main(void)
{
    ...
    vertex_list vl_square = find_g_square_list(vl);
    print_graph(vl_square);
    ...
    int** matrix_square = find_g_square_matrix(matrix);
    for (int i = 0; i < 6; ++i)
    {
        for (int j = 0; j < 6; ++j)
